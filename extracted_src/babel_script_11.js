...
function App() {
  ...
  const [screen, setScreen] = useAppState(() => localStorage.getItem("kv_hifi_screen") || "home");
  const [cart, setCart] = useAppState(3);
  const [toast, setToast] = useAppState(null);
  const hist = useAppRef([]);
  const toastTimer = useAppRef(null);

  useAppEffect(() => { localStorage.setItem("kv_hifi_screen", screen); }, [screen]);
  ...
  else if (screen === "category" && window.CategoryScreen) body = <CategoryScreen go={go} onAdd={addToCart} />;
  else if (screen === "catalog" && window.CatalogScreen) body = <CatalogScreen go={go} onAdd={addToCart} />;
  else if (screen === "product" && window.ProductScreen) body = <ProductScreen go={go} onAdd={addToCart} />;
  else if (screen === "cart" && window.CartScreen) body = <CartScreen go={go} cart={cart} setCart={setCart} />;
  ...
  return (
    <div style={accentVars}>
        ...
        <div className="proto-inner">
          <div className="proto-brand">
            ...
            <div>
              ...
              <div className="proto-sub">Преміальний B2B · клікабельний потік</div>
            </div>
          </div>
          <div className="proto-tabs">
            <button className="proto-back" onClick={back} disabled={!hist.current.length} title="Назад">←</button>
            {SCREENS.map(s => (
              <button key={s.id} className={"proto-tab" + (s.id === screen ? " is-active" : "")} onClick={() => go(s.id)}>{s.label}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* toast */}
      {toast && (
        <div style={{ position: "fixed", bottom: 26, left: "50%", transform: "translateX(-50%)", zIndex: 80, background: "var(--ink)", color: "var(--on-dark)", padding: "13px 22px", borderRadius: "var(--r-md)", boxShadow: "var(--sh-lg)", display: "flex", alignItems: "center", gap: 10, fontWeight: 600, fontSize: 14 }} className="reveal">
