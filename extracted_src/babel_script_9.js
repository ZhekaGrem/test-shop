// hifi-product.jsx — product detail page
const { useState: useProductState } = React;

function Gallery({ p }) {
  const [active, setActive] = useProductState(0);
...
function QtyStepper() {
  const [q, setQ] = useProductState(1);
  return (
...
function ProductTabs({ p }) {
  const [tab, setTab] = useProductState(0);
      ...
      <div className="wrap" style={{ paddingTop: 24, paddingBottom: 20 }}>
          ...
          <div style={{ position: "sticky", top: 96, display: "flex", flexDirection: "column", gap: 16 }} className="buy-box">
            <div className="card" style={{ padding: 22 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                <QtyStepper />
                <button className="btn btn-accent btn-lg" style={{ flexGrow: 1 }} onClick={onAdd}>{ICONS.cart} У кошик</button>
              </div>
              <button className="btn btn-primary btn-lg btn-block" onClick={() => { onAdd(); go("cart"); }}>Купити в 1 клік</button>
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button className="btn btn-soft btn-sm" style={{ flexGrow: 1 }}>{ICONS.compare} Порівняти</button>
                <button className="btn btn-soft btn-sm" style={{ flexGrow: 1 }}>{ICONS.heart} В обране</button>
              </div>
            </div>
        ...
        <div style={{ marginTop: 56 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }} className="prod-grid">
            {related.map((p, i) => <ProductCard key={i} p={p} go={go} onAdd={onAdd} />)}
          </div>
        </div>
      </div>
    </div>
