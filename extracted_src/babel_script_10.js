// hifi-cart.jsx — cart with checkout stepper + summary
const { useState: useCartState } = React;
    ...
    <div style={{ display: "flex", gap: 16, padding: "18px 0", borderBottom: "1px solid var(--line)", alignItems: "center" }} className="cart-row">
      ...
      <div style={{ flexGrow: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <a style={{ fontSize: 12.5, color: "var(--muted)", cursor: "pointer" }} className="crumb-link">В обране</a>
...
function CartScreen({ go }) {
  const [qtys, setQtys] = useCartState(CART_ITEMS.map(i => i.qty));
  ...
  function setQ(i, v) { setQtys(qtys.map((q, k) => k === i ? v : q)); }
          ...
          <div>
            ...
            <div style={{ marginTop: 28 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }} className="prod-grid">
                {PRODUCTS.slice(4, 8).map((p, i) => <ProductCard key={i} p={p} go={go} onAdd={() => {}} />)}
              </div>
            </div>
          </div>
